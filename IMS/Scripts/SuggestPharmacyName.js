    ...
    if (e.length < 3) {
        ...
        if (key == 40 || key == 38) {
            ...
            if (searchReq.readyState == 4 || searchReq.readyState == 0) {
                var str = escape(document.getElementById('MainContent_MultipleVendorsSelectPopup_txtSearch').value);
                strOriginal = str;
                searchReq.open("GET", 'PharmacyNameResult.aspx?search=' + str, true);
...
function scrollOver(div_value) {
    ...
    div_value.className = 'suggest_link_over';
    document.getElementById('MainContent_MultipleVendorsSelectPopup_txtSearch').value = div_value.innerHTML;
}
...
function setSearch(value) {
    ...
    document.getElementById('MainContent_MultipleVendorsSelectPopup_txtSearch').value = value;
    ...
    if (key == 40) {
        ...
        if (currentDivId == -1) {
            ...
            if (currentDivId == maxDivId) {
                ...
                currentDivId = -1;
                suggestOut(a)
                document.getElementById('MainContent_MultipleVendorsSelectPopup_txtSearch').value = strOriginal;
            }
            else {
